...
if (isset($_COOKIE["user"]) && $_COOKIE["user"]>0){
	$q = $mysqli->query("SELECT `id`, `email`, `name`, `surname`, `sex`, UNIX_TIMESTAMP(`dob`) AS dob, UNIX_TIMESTAMP(`reg_date`) AS reg_date, `ava`, `admin`, `address`, `phone`, `skype`, `icq` FROM users WHERE id=".intval($_COOKIE["user"])." LIMIT 1");
	if($q && $q->num_rows===1){
		$r = $q->fetch_assoc();
