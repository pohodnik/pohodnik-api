...
$is_vacant_route = $_POST['is_vacant_route'] == 'true' ? 1 : 0;

if ($d1 >= $d2) {
    exit(json_encode(array("error" => "Дата начала должна быть раньше даты завершения")));
}
...
if ($id > 0) {
    if ($mysqli->query("UPDATE `hiking` SET 
        ...
        exit(json_encode(array("error" => "Ошибка обновления . \r\n" . $mysqli->error)));
    }
} else {
    ...
    if ($mysqli->query("INSERT INTO `hiking` SET 
        ...
        exit(json_encode(array("error" => "Ошибка добавления. \r\n" . $mysqli->error)));
    }
}
