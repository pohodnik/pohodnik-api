...
while ($r = $qb->fetch_assoc()) {
  if (!empty($r['params'])) {
    $r['params'] = explode('œ', $r['params']);
  } else {
    $r['params'] = null;
  }
  if (!empty($r['dir_params'])) {
    $r['dir_params'] = explode('œ', $r['dir_params']);
  } else {
    $r['dir_params'] = null;
  }
  if (!empty($r['variants'])) {
    $r['variants'] =  array_map('variant', explode('æ', $r['variants']));
  } else {
    $r['variants'] = null;
  }
  if (!empty($r['answer'])) {
    $r['answer'] =  array_map('variant', explode('æ', $r['answer']));
  } else {
    $r['answer'] = null;
  }
