...
$q = $mysqli->query("
    INSERT INTO
        ...
        ROUND(SUM(recipes_structure.amount * (hiking_menu.сorrection_coeff_pct / 100)),2) AS need_amount,
        GROUP_CONCAT(
            CONCAT_WS(
                '|',
                recipes.name,
                hiking_menu.date,
                ROUND(recipes_structure.amount * (hiking_menu.сorrection_coeff_pct / 100),2)
            )
        ) AS usages
    FROM hiking_menu
