...
in`id`,clude("../../blocks/global.php"); //Только для авторизованных
$result = array();
$id = intval($_POST['id']);
...
if(!$q) die(err(array('error'=>$mysqli->error, 'q' => $z)));
...
if(!$q) die(err(array('error'=>$mysqli->error,'type'=> 'route objects',  'q' => $z)));

$z = "INSERT INTO `route_regions`
    (`id_route`, `id_region`)
SELECT  
    {$new_route_id} as `id_route`, `id_region`
FROM `route_regions` WHERE id_route={$id}";
$q = $mysqli -> query($z);
