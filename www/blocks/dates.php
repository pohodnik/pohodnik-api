...
function smartDate($time, $show_time = true){
	...
	if (date('YmdH', $time) === date('YmdH')) {
		return $show_time ? date('H:i', $time) : 'Сегодня';
	} else if (date('Ymd', $time) === date('Ymd')) {
		return $show_time ? "Сегодня в ".date('H:i', $time) : "Сегодня";
	} else if(date('Ymd', $time) === date('Ymd', time()-86400)) {
		return $show_time ? "Вчера в ".date('H:i', $time) : "Вчера";
	} else if(date('Ymd', $time) === date('Ymd', time()+86400)) {
		...
		return $show_time ? date('d.m.Y в H:i', $time) : date('d.m.Y', $time);
	}
}
?>